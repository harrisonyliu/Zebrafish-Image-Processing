...
z_proj = zeros(size(neuron_crop,1), size(neuron_crop,2));
for i = 1:size(neuron_crop,1)
    for j = 1:size(neuron_crop,2)
        z_proj(i,j) = max(neuron_crop(i,j,:));
    end
end
...
fname_neuron = fullfile('F:\Zebrafish neuron projections', date, [handles.well_name '.tif']);
