...
filename = fullfile('F:\CellProfiler Output\2016.05.23 Jeff redo','2016.05.23 Plate 1Image.csv');
...
platemap.posctrl_lo = createWellGroups('A', 'H', 1, 1);
platemap.negctrl_lo = createWellGroups('A', 'H', 12, 12);
platemap.threeMA_lo = createWellGroups('A', 'H', 2, 2);
platemap.Rufin_lo = createWellGroups('A', 'H', 3, 3);
platemap.Aemp_lo = createWellGroups('A', 'H', 4, 4);
platemap.Quercetin_lo = createWellGroups('A', 'H', 9, 9);
platemap.cmpd7_lo = createWellGroups('A', 'H', 10, 10);
...
%% Plate 2
filename = fullfile('F:\CellProfiler Output\2016.05.23 Jeff redo','2016.05.23 Plate 2Image.csv');
plate_struct = separateReplicatePlates(filename,'Count_Neurons',...
    'Intensity_TotalIntensity_ghettoconv',...
    'Intensity_TotalIntensity_position_measure',...
    'Intensity_TotalIntensity_eyes_removed_cropped_Neurons',...
...
% platemap.onetwentyNAC = createWellGroups('B', 'B', 1, 12);
clear platemap;
platemap.posctrl_hi = createWellGroups('A', 'H', 1, 1);
platemap.negctrl_hi = createWellGroups('A', 'H', 12, 12);
platemap.threeMA_hi = createWellGroups('A', 'H', 2, 2);
platemap.Rufin_hi = createWellGroups('A', 'H', 3, 3);
platemap.Aemp_hi = createWellGroups('A', 'H', 4, 4);
platemap.Quercetin_hi = createWellGroups('A', 'H', 9, 9);
platemap.cmpd7_hi = createWellGroups('A', 'H', 10, 10);
